...
def login():
    if request.method == 'POST':
        ...
        # Busca o usuário e os dados do cliente vinculado a ele
        res = supabase.table('users').select('*, clients(name)').eq('username', username).eq('password', password).execute()
        
        if res.data:
            user = res.data[0]
            # Salva na memória do Flask (Sessão) os dados de quem logou
...
@app.route('/logout')
def logout():
    session.clear() # Limpa a memória e desloga
    return redirect(url_for('login'))

# --- ROTAS DO SISTEMA (AGORA PROTEGIDAS E FILTRADAS) ---

@app.route('/')
@login_required # <--- Essa tag garante que só quem logou entra aqui
def index():
    selected_dept_id = request.args.get('dept_id')
    client_id = session['client_id'] # Pega o ID do cliente logado
    
    # Busca dados APENAS deste cliente
    ...
    return render_template('index.html', 
                           ...
                           session=session) # Passamos a sessão para o HTML
...
def update_operational():
    ...
    if item_type == 'kr_checkin':
        new_value = float(request.form.get('current_value'))
        supabase.table('key_results').update({'current_value': new_value}).eq('id', item_id).execute()
